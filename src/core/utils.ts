 ...
 * @param config 配置项
 */
export function c2n(config: NodeConfig | NodeConfig[]): NodeConfig {
 ...
 * @param config
 */
export function getDefaultValues(config: NodeConfig | NodeConfig[]) {
  const data = {};
  const process = (node: NodeConfig, parentName: string) => {
    // 如果当前节点的name匹配的是`[]`，需要将name匹配成第0项
    const _name = node.name.replace(/\[\]$/g, '[0]');
    const fullyName = resolveName(parentName, _name);

    // 如果节点本身有 children，是不会直接set值的，会向下层递归set
    if (node.children) {
...
export const required = (v: string) => (v && v.length ? undefined : '字段不能为空');
